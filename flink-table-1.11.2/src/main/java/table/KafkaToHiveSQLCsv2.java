...
public class KafkaToHiveSQLCsv2 {
    static final String kafkaDDL = "CREATE TABLE KafkaSQLCsv ( " +
            "user_id STRING, " +
            "order_amount DOUBLE, " +
            //通过使用UDF将消息中的字段转换为事件时间，并用于生成水位线
            "log_ts_ori STRING, " +
            "log_ts AS TO_TIMESTAMP(MYUDF(log_ts_ori)), " +
            ...
            "'csv.field-delimiter'='|'" +
            ")";
    static final String hiveDDL = "CREATE TABLE HiveSQLCsv (" +
            "user_id STRING," +
            "order_amount DOUBLE" +
            ...
            ")";
    static final String DML = "INSERT INTO TABLE HiveSQLCsv " +
            //可以通过人工指定字段进行输出
    ...
    public static class MYUDF extends ScalarFunction {
        public String eval(String timeMillis) {
            return ZonedDateTime.ofInstant(Instant.ofEpochMilli(Long.parseLong(timeMillis)), ZoneId.of("Asia/Shanghai"))
